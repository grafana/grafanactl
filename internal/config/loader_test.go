...
func TestLoad_explicitFile(t *testing.T) {
	...
	cfg, err := config.Load(testutils.NullLogger(), config.ExplicitConfigFile("./testdata/config.yaml"))
	req.NoError(err)
...
func TestLoad_explicitFile_notFound(t *testing.T) {
	...
	_, err := config.Load(testutils.NullLogger(), config.ExplicitConfigFile("./testdata/does-not-exist.yaml"))
	req.Error(err)
...
func TestLoad_standardLocation_noExistingConfig(t *testing.T) {
	...
	cfg, err := config.Load(testutils.NullLogger(), config.StandardLocation())
	req.NoError(err)
...
func TestLoad_withOverride(t *testing.T) {
	...
	cfg, err := config.Load(testutils.NullLogger(), config.ExplicitConfigFile("./testdata/config.yaml"), func(cfg *config.Config) error {
...
this-field-is-invalid: []`
	...
	_, err := config.Load(testutils.NullLogger(), config.ExplicitConfigFile(configFile))
	req.Error(err)
	req.ErrorAs(err, &fail.DetailedError{}, "a detailed error is returned")
...
func TestWrite(t *testing.T) {
	...
	err := config.Write(testutils.NullLogger(), config.ExplicitConfigFile(configFile), cfg)
	req.NoError(err)
